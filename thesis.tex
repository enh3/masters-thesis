...
If one is careful to take into consideration the nuances of the original interface, integrating them with their ideas in a way that complements, rather than fights the natural tendencies of the instrument, the result can be an enduring homage, and a continuation of a tradition that crosses both culture and time.
...
Beilharz align's themself with Machover's approach to interactivity, employing a “Reactive Space” that incorporates not only sonic, but visual components\footcite[352]{beilharz_hyper-shaku_2006}. 
Using wireless motion sensors and computer vision, she explores two distinct though complementary approaches to treating and mapping this data using Max/MSP and Jitter. 
On one hand, a Cellular Automata function, which is used to create generative algorithms for the visual display, and on the other, a Neural Oscillator Network for the audio processing, which controls granular synthesis of the shakuhachi. 

Gestural data is central to the hyper-shaku's functionality\footcite[356]{beilharz_hyper-shaku_2006}. 
...
Despite this, her pursuit of improvisation with a high degree of freedom, open forms, and interactivity is in line with the practices of Machover, Palacio-Quintin, and Beilharz.
...
One can imagine, for instance, coupling the Positif to a synthesized sound, simply by sending its MIDI output to a laptop running synthesis software, or an analog synthesizer, essentially playing both acoustic and synthetic from the same keyboard. 
