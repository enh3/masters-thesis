...
\makeatletter
\@addtoreset{example}{section} % Reset the example counter at the section level
\renewcommand{\theexample}{\arabic{example}} % Simple numbering (1, 2, 3...)
%\def\captionheadfont@{\scshape} % Same as the figure
...
%\def\fnum@example{\figurename~\theexample} % Format like figure (e.g., "Example 1")
%\def\figurename{FIGURE} % Ensures the "Example" label is similar to "Fig."
\makeatother
...
\customincludegraphics[]{g7alt}{A classic G7alt chord on the left, and my variation with a \sh7 on the right.}
